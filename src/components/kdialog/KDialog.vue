<script lang="ts" setup>
    ...
    const props = defineProps({
        // 提示框类型：confirm、warning、error、info
        type: {
            type: String,
            default: "info"
        },
    ...
    const isConfirmBtnShow = computed(() => {
        if(props.footerBtn === "both" || props.footerBtn === "confirm") 
            return true;
        else
            return false;
    })
    ...
    const isCancelBtnShow = computed(() => {
        if(props.footerBtn === "both" || props.footerBtn === "cancel") 
            return true;
        else 
            return false;
    })
...
<template>
    <div class="dialog" :style="[sizeCss]">
        <!-- 标题 -->
        <div class="title" :style="[bgColorCss]">
            <span>{{ props.titleText }}</span>
            <k-button class="closeBtn" v-if="closable" type="default">×</k-button>
        </div>
        <div class="base">
            <!-- 内容 -->
            <div class="content">
                ...
                <div class="info">{{ props.contentText }}</div>
            </div>
            <!-- 底部按钮 -->
            <div class="footerBox" v-if="!(props.footerBtn === 'null')">
                <k-button class="footerBtn confirmBtn" v-if = "isConfirmBtnShow">{{ props.confirmBtnText }}</k-button>
                <k-button class="footerBtn cancelBtn" v-if = "isCancelBtnShow">{{ props.cancelBtnText }}</k-button>
            </div>
        </div>
    </div>
</template>
